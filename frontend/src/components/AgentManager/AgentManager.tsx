    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-app-surface border border-app-border rounded-xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-card-md">
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-app-border">
          <h2 className="text-sm font-semibold text-app-text">Manage Agents</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-6">
          ...
          <section>
            ...
            <div className="flex flex-col gap-2">
              {builtIn.map(agent => (
                <AgentRow key={agent.type} agent={agent} readOnly />
              ))}
            </div>
          </section>
...
function AgentRow({
  ...
  return (
    <div className="flex items-center gap-3 bg-app-col border border-app-border rounded-lg px-4 py-3">
      ...
      <span className="text-[10px] text-app-muted truncate max-w-28">{agent.model?.split('/').pop()}</span>
      {!readOnly && onDelete && (
        <button
          onClick={onDelete}
          className="p-1.5 rounded hover:bg-red-500/10 text-app-muted hover:text-red-500 transition-colors ml-1"
        >
          <Trash2 size={12} />
        </button>
      )}
